...
from tensorflow.keras.layers import Input, Embedding, MultiHeadAttention, Dense, GlobalAvgPool1D, Conv1D, BatchNormalization, Activation, Dropout, LayerNormalization
...
class Trainer:
    ...
    def load_CIRCLE_dataset(self, sgRNA_type_index=None):
        ...
        if sgRNA_type_index == None: # 不按照sgRNA分组，随机分
            ...
            for i, row in circle_dataset.iterrows():
                ...
                label = row['label']
                pair_code = encode_by_base_pair_vocabulary(on_target_seq=gRNA_seq, off_target_seq=target_seq)
            ...
            if len(self.ten_grna_fold_CIRLCE[0]["features"]) != 0: # 如果已经分好了就直接从里面取
                ...
                for i in range(10):
                    ...
                    print("[INFO] %s-th-grna-fold set, labels shape=%s, and positive samples number = %s"%(i, str(self.ten_grna_fold_CIRLCE[i]["labels"].shape), len(self.ten_grna_fold_CIRLCE[i]["labels"][self.ten_grna_fold_CIRLCE[i]["labels"]>0])))
                    if i != sgRNA_type_index:
        ...
        print("[INFO] train_labels.shape = %s, and positive samples number = %s"%(self.train_labels.shape, len(self.train_labels[self.train_labels>0])))
        print("[INFO] validation_features.shape = %s"%str(self.validation_features.shape))
        print("[INFO] validation_labels.shape = %s, and positive samples number = %s"%(self.validation_labels.shape, len(self.validation_labels[self.validation_labels>0])))
    ...
    def train_model(self):
        ...
        # 2 * Encoder Layer
        encoder_output = self.add_encoder_layer(encoder_positional_encoding)
        encoder_output = self.add_encoder_layer(encoder_output)
        ...
        # 2 * Decoder Layer
        decoder_output = self.add_decoder_layer(decoder_positional_encoding, encoder_output)
        decoder_output = self.add_decoder_layer(decoder_output, encoder_output)

        main_branch = GlobalAvgPool1D()(decoder_output)
    ...
    def compute_auroc_and_auprc(self, model):
        ...
        # print(y_pred)
        # print(max(y_pred))
        # print(len(y_pred[y_pred==0.37903923]))
        pred_labels = list()
        for i in y_pred:
            if i >= 0.5:
...
if __name__ == "__main__":
    ...
    for i in range(10):
        trainer.load_CIRCLE_dataset(sgRNA_type_index=i)
...
# average_test_loss=0.40217322409152984, average_test_mae=0.322709658741951, average_test_acc=0.9881121814250946
