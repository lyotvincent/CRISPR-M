...
class Trainer:
    ...
    def train_model(self):
        ...
        # 2 * Encoder Layer
        encoder_output = add_encoder_layer(encoder_positional_encoding, num_heads=4, key_dim=6, units_dim=self.EMBED_SIZE, model_dim=self.EMBED_SIZE)
        encoder_output = add_encoder_layer(encoder_output, num_heads=4, key_dim=6, units_dim=self.EMBED_SIZE, model_dim=self.EMBED_SIZE)
        ...
        # 2 * Decoder Layer
        decoder_output = add_decoder_layer(decoder_positional_encoding, encoder_output, num_heads=4, key_dim=6, units_dim=self.EMBED_SIZE, model_dim=self.EMBED_SIZE)
        decoder_output = add_decoder_layer(decoder_output, encoder_output, num_heads=4, key_dim=6, units_dim=self.EMBED_SIZE, model_dim=self.EMBED_SIZE)
