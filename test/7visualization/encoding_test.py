...
from data_preprocessing_utils import load_CIRCLE_dataset_encoded_by_both_base_and_base_pair, load_I_2_dataset_encoded_by_both_base_and_base_pair, load_PKD_encoded_by_both_base_and_base_pair, load_SITE_encoded_by_both_base_and_base_pair, load_GUIDE_II_encoded_by_both_base_and_base_pair, load_GUIDE_III_encoded_by_both_base_and_base_pair
...
class Trainer:
    ...
    def train_model(self):
        ...
        accuracy, precision, recall, f1, fbeta, auroc_skl, auprc_skl, auroc_by_auc, auprc_by_auc, spearman_corr_by_pred_score, spearman_corr_by_pred_labels = compute_auroc_and_auprc(model=model, out_dim=1, test_features=[self.validation_features, self.validation_feature_ont, self.validation_feature_offt], test_labels=self.validation_labels)
        return test_loss, test_acc, auroc, auprc, accuracy, precision, recall, f1, fbeta, auroc_skl, auprc_skl, auroc_by_auc, auprc_by_auc, spearman_corr_by_pred_score, spearman_corr_by_pred_labels

if __name__ == "__main__":
    time1 = time.time()

    trainer = Trainer()
    train_features_1, train_feature_on_1, train_feature_off_1, train_labels_1 = load_CIRCLE_dataset_encoded_by_both_base_and_base_pair()
    train_features_2, train_feature_on_2, train_feature_off_2, train_labels_2 = load_SITE_encoded_by_both_base_and_base_pair()
    val_features_1, val_feature_on_1, val_feature_off_1, val_labels_1 = load_I_2_dataset_encoded_by_both_base_and_base_pair()
    val_features_2, val_feature_on_2, val_feature_off_2, val_labels_2 = load_PKD_encoded_by_both_base_and_base_pair()
    val_features_3, val_feature_on_3, val_feature_off_3, val_labels_3 = load_GUIDE_II_encoded_by_both_base_and_base_pair()
    val_features_4, val_feature_on_4, val_feature_off_4, val_labels_4 = load_GUIDE_III_encoded_by_both_base_and_base_pair()
    trainer.train_features, trainer.train_feature_ont, trainer.train_feature_offt, trainer.train_labels = np.concatenate([train_features_1, train_features_2]), np.concatenate([train_feature_on_1, train_feature_on_2]), np.concatenate([train_feature_off_1, train_feature_off_2]), np.concatenate([train_labels_1, train_labels_2])
    trainer.validation_features, trainer.validation_feature_ont, trainer.validation_feature_offt, trainer.validation_labels = np.concatenate([val_features_1, val_features_2, val_features_3, val_features_4]), np.concatenate([val_feature_on_1, val_feature_on_2, val_features_3, val_features_4]), np.concatenate([val_feature_off_1, val_feature_off_2, val_feature_off_3, val_feature_off_4]), np.concatenate([val_labels_1, val_labels_2, val_labels_3, val_labels_4])
    ...
    print("test_loss=%s, test_acc=%s, auroc=%s, auprc=%s, accuracy=%s, precision=%s, recall=%s, f1=%s, fbeta=%s, auroc_skl=%s, auprc_skl=%s, auroc_by_auc=%s, auprc_by_auc=%s, spearman_corr_by_pred_score=%s, spearman_corr_by_pred_labels=%s" % (test_loss, test_acc, auroc, auprc, accuracy, precision, recall, f1, fbeta, auroc_skl, auprc_skl, auroc_by_auc, auprc_by_auc, spearman_corr_by_pred_score, spearman_corr_by_pred_labels))
