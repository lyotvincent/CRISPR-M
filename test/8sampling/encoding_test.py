...
if __name__ == "__main__":
    ...
    ## oversampling
    # positive_samples_index = (trainer.train_labels>0)
    # negative_samples_index = (trainer.train_labels<=0)
    # positive_train_features = trainer.train_features[positive_samples_index].tolist()*77
    # positive_train_feature_ont = trainer.train_feature_ont[positive_samples_index].tolist()*77
    # positive_train_feature_offt = trainer.train_feature_offt[positive_samples_index].tolist()*77
    # positive_train_feature_labels = trainer.train_labels[positive_samples_index].tolist()*77
    # trainer.train_features = trainer.train_features.tolist()
    # trainer.train_feature_ont = trainer.train_feature_ont.tolist()
    # trainer.train_feature_offt = trainer.train_feature_offt.tolist()
    # trainer.train_labels = trainer.train_labels.tolist()
    # trainer.train_features.extend(positive_train_features)
    # trainer.train_feature_ont.extend(positive_train_feature_ont)
    # trainer.train_feature_offt.extend(positive_train_feature_offt)
    # trainer.train_labels.extend(positive_train_feature_labels)
    # trainer.train_features = np.array(trainer.train_features)
    # trainer.train_feature_ont = np.array(trainer.train_feature_ont)
    # trainer.train_feature_offt = np.array(trainer.train_feature_offt)
    # trainer.train_labels = np.array(trainer.train_labels)
    ## undersampling
    positive_samples_index = (trainer.train_labels>0)
    negative_samples_index = (trainer.train_labels<=0)
    positive_train_features = trainer.train_features[positive_samples_index].tolist()
    positive_train_feature_ont = trainer.train_feature_ont[positive_samples_index].tolist()
    positive_train_feature_offt = trainer.train_feature_offt[positive_samples_index].tolist()
    positive_train_feature_labels = trainer.train_labels[positive_samples_index].tolist()
    negative_train_features = trainer.train_features[negative_samples_index].tolist()
    negative_train_feature_ont = trainer.train_feature_ont[negative_samples_index].tolist()
    negative_train_feature_offt = trainer.train_feature_offt[negative_samples_index].tolist()
    negative_train_feature_labels = trainer.train_labels[negative_samples_index].tolist()
    negative_train_features = random.sample(negative_train_features, len(positive_train_feature_labels))
    negative_train_feature_ont = random.sample(negative_train_feature_ont, len(positive_train_feature_labels))
    negative_train_feature_offt = random.sample(negative_train_feature_offt, len(positive_train_feature_labels))
    negative_train_feature_labels = random.sample(negative_train_feature_labels, len(positive_train_feature_labels))
    trainer.train_features = np.array(positive_train_features+negative_train_features)
    trainer.train_feature_ont = np.array(positive_train_feature_ont+negative_train_feature_ont)
    trainer.train_feature_offt = np.array(positive_train_feature_offt+negative_train_feature_offt)
    trainer.train_labels = np.array(positive_train_feature_labels+negative_train_feature_labels)

    print("train_features shape: %s"%str(trainer.train_features.shape))
    print("train_feature_ont shape: %s"%str(trainer.train_feature_ont.shape))
    print("train_feature_offt shape: %s"%str(trainer.train_feature_offt.shape))
    print("train_labels shape: %s"%str(trainer.train_labels.shape))
